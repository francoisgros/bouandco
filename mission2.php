...
$result = $db->query('SELECT membre FROM bataille2 WHERE membre LIKE \'' . str_replace('\'', '\'\'', stripslashes($_GET['membre'])) . '\'');
...
<table border="1">
    ...
    $results = $db->query('SELECT * FROM bataille2 WHERE membre LIKE \'' . str_replace('\'', '\'\'', $membre['membre']) . '\' ORDER BY ville, cible, typecible, niv DESC');
    $cible = '';
